...
contract DEX is IUniswapV2Router02 {
    using SafeMath for uint;

    address public immutable override factory;
    address public immutable override WETH;
    address public pairaddress;
    uint constant DEADLINE = 7 days; 
    ...
    constructor(address _factory, address _WETH, address _pairaddress) public {
        factory = _factory;
        WETH = _WETH;
    ...
    function sellETH(uint256 sellAmount,address buyToken,uint256 minBuyAmount) external payable {
        //path：WETH -> buyToken
        address[] memory path = new address[](2);
        path[0] = address(WETH);
        path[1] = address(buyToken);
        swapExactTokensForTokens(sellAmount, minBuyAmount, path, msg.sender, block.timestamp+DEADLINE);
    }
